...
class HotstarDownloader:
                ...
                try:
                    ...
                    if '#EXTM3U' in body:
                        ...
                        if variants:
                            ...
                            await self.auto_download(best_variant, self.headers, timestamp)
    ...
    def generate_filename(self, url, page_url):
        ...
        if page_url and 'hotstar.com' in page_url:
            ...
            clean = []
            for seg in segments:
                if not re.match(r'^\d+$', seg) and seg not in ['in', 'shows', 'movies', 'watch']:
                    clean.append(seg)
            if clean:
                name = '_'.join(clean[:3])  # Take first 3 meaningful parts
                return f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
        
        # Fallback to timestamp
        return f"hotstar_video_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
    
    async def auto_download(self, variant, headers, timestamp):
        """Automatically download using ffmpeg"""
        # Generate filename
        if self.page_url and 'hotstar.com' in self.page_url:
            ...
            for seg in segments:
                if not re.match(r'^\d+$', seg) and seg not in ['in', 'shows', 'movies', 'watch']:
                    clean.append(seg)
            if clean:
                filename = f"{'_'.join(clean[:3])}_{timestamp}.mp4"
            else:
    ...
    async def run(self, url, headless=False):
        ...
        async with async_playwright() as p:
            ...
            try:
                # Launch browser with DRM support
